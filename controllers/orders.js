...
const isNotValidOrder = (orders) => {
  if (isUndefined(orders) || !Array.isArray(orders)) {
    // 檢查orders 是否為一個陣列
    return true;
  }
  //逐筆檢查陣列裡的每一個訂單項目
...
const postOrder = async (req, res, next) => {
  try {
    ...
    if (
      ...
      isNotValidString(user.name) ||
      !userNameReg.test(user.name) ||
      !Object.prototype.hasOwnProperty.call(user, "tel") ||
      isNotValidString(user.tel) ||
      !telReg.test(user.tel) ||
      !Object.prototype.hasOwnProperty.call(user, "address") ||
      isNotValidString(user.address) ||
      user.address.length > 30 ||
      isNotValidOrder(orders) ||
      isNotValidInteger(paymentMethods) ||
      !paymentMethodReg.test(String(paymentMethods))
    ) {
      logger.warn("欄位未填寫正確");
      res.status(400).json({
        status: "failed",
    ...
    const newOrder = await orderRepostory.save(
      orderRepostory.create({
        users_id: id,
        name: user.name,
        tel: user.tel,
    ...
    const linkResult = await orderLinkProductRepository.insert(
      orders.map((order) => ({
